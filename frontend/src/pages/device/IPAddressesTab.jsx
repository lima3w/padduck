      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow overflow-hidden">
        ...
        <table className="w-full text-sm">
          <thead className="bg-gray-50 dark:bg-gray-700 border-b dark:border-gray-600">
            <tr>
              <th className="text-left px-4 py-3 text-gray-600 dark:text-gray-300 font-medium">{t('deviceIp.address')}</th>
              <th className="text-left px-4 py-3 text-gray-600 dark:text-gray-300 font-medium">{t('deviceIp.interface')}</th>
          ...
          <tbody>
            {ipAddresses.length === 0 && (
              <tr><td colSpan={5} className="px-4 py-6 text-center text-gray-400">{t('deviceIp.noIpsAssociated')}</td></tr>
            )}
            {ipAddresses.map(ip => (
              <tr key={ip.id} className="border-b dark:border-gray-700 last:border-0 hover:bg-gray-50 dark:hover:bg-gray-700/30">
                ...
                <td className="px-4 py-3">
                  {ip.isPrimary && (
                    <span className="inline-block px-2 py-0.5 bg-blue-100 text-blue-700 text-xs font-medium rounded">{t('deviceIp.primary')}</span>
                  )}
                </td>
                ...
                <td className="px-4 py-3 text-right">
                  {deleteIpConfirm === ip.id ? (
                    <span className="space-x-2">
                      ...
                      <button onClick={() => onDisassociateIP(ip.id)} className="text-red-600 hover:text-red-800 text-xs font-medium">{t('common.yes')}</button>
                      <button onClick={() => setDeleteIpConfirm(null)} className="text-gray-400 hover:text-gray-600 text-xs">{t('common.no')}</button>
                    </span>
                  ) : (
                    <button onClick={() => setDeleteIpConfirm(ip.id)} className="text-gray-400 hover:text-red-600 text-xs">{t('deviceIp.remove')}</button>
                  )}
                </td>
              </tr>
            ))}
