...
export default function DelegationsTab({ subnetId }) {
  ...
  const loadDelegations = useCallback(async () => {
    try {
      ...
      setError(t('delegations.loadError'))
    } finally {
      setLoading(false)
    }
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      setError(err.response?.data?.error || t('delegations.saveError'))
    } finally {
      setSaving(false)
    }
  }
  ...
  async function handleDelete(id) {
    try {
      ...
      setError(t('delegations.deleteError'))
    }
  }
  ...
  if (loading) return <PageSpinner message={t('delegations.loading')} />

  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-lg font-semibold text-gray-800 dark:text-gray-200">{t('delegations.title')}</h2>
        <button onClick={openCreate} className="px-3 py-1.5 bg-blue-600 text-white rounded text-sm hover:bg-blue-700">{t('delegations.addDelegation')}</button>
      </div>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow overflow-hidden">
        ...
        <table className="w-full text-sm">
          <thead className="bg-gray-50 dark:bg-gray-700 border-b dark:border-gray-600">
            <tr>
              <th className="text-left px-4 py-3 text-gray-600 dark:text-gray-300 font-medium">{t('delegations.delegatedPrefix')}</th>
              <th className="text-left px-4 py-3 text-gray-600 dark:text-gray-300 font-medium">{t('delegations.descriptionDevice')}</th>
              <th className="text-left px-4 py-3 text-gray-600 dark:text-gray-300 font-medium">{t('delegations.expiresAt')}</th>
          ...
          <tbody>
            {delegations.length === 0 && (
              <EmptyRow colSpan={5} message={t('delegations.noDelegationsYet')} />
            )}
            {delegations.map(d => (
              <tr key={d.id} className="border-b dark:border-gray-700 last:border-0 hover:bg-gray-50 dark:hover:bg-gray-700/30">
                ...
                <td className="px-4 py-3 text-gray-600 dark:text-gray-400">{d.delegatedToDescription || (d.delegatedToDeviceId ? t('delegations.deviceHash', { id: d.delegatedToDeviceId }) : '—')}</td>
                ...
                <td className="px-4 py-3">
                  <span className={`inline-block px-2 py-0.5 rounded text-xs font-medium ${d.isExpired ? 'bg-red-100 text-red-700' : 'bg-green-100 text-green-700'}`}>
                    {d.isExpired ? t('delegations.expired') : t('delegations.active')}
                  </span>
                </td>
                <td className="px-4 py-3 text-right space-x-2">
                  ...
                  {deleteConfirm === d.id ? (
                    <>
                      <span className="text-red-600 text-xs">{t('delegations.confirmDelete')}</span>
      ...
      {modal && (
        <Modal title={modal === 'create' ? t('delegations.newDelegationModalTitle') : t('delegations.editDelegationModalTitle')} onClose={() => setModal(null)}>
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <label className={labelClass}>{t('delegations.delegatedPrefix')}</label>
            ...
            <div>
              <label className={labelClass}>{t('delegations.deviceIdOptional')}</label>
              <input type="number" className={inputClass} placeholder={t('delegations.deviceIdPlaceholder')} value={form.delegatedToDeviceId} onChange={e => setForm(f => ({ ...f, delegatedToDeviceId: e.target.value }))} />
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className={labelClass}>{t('delegations.validLifetime')}</label>
              ...
              <div>
                <label className={labelClass}>{t('delegations.preferredLifetime')}</label>
