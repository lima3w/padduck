...
import { getSubnet, getIPAddresses, createIPAddress, assignIPAddress, releaseIPAddress, deleteIPAddress, searchIPAddresses, getTags, updateIPMeta } from '../api/client'
    ...
    <div>
      ...
      <div className="mb-4 space-y-2">
        <form onSubmit={handleSearch} className="flex gap-2">
          ...
          {(searchQuery || searchStatus || Object.values(advFilters).some(Boolean)) && (
            <button
              type="button"
      ...
      <div className="bg-white rounded-lg shadow overflow-hidden">
        <table className="w-full text-sm">
          <thead className="bg-gray-50 border-b">
            <tr>
              {col('address') && <th className="text-left px-4 py-3 text-gray-600 font-medium">Address</th>}
              {col('hostname') && <th className="text-left px-4 py-3 text-gray-600 font-medium">Hostname</th>}
              {col('status') && <th className="text-left px-4 py-3 text-gray-600 font-medium">Status</th>}
              {col('tag') && <th className="text-left px-4 py-3 text-gray-600 font-medium">Tag</th>}
              {col('assigned_to') && <th className="text-left px-4 py-3 text-gray-600 font-medium">Assigned To</th>}
              {col('mac_address') && <th className="text-left px-4 py-3 text-gray-600 font-medium">MAC Address</th>}
              {col('ptr_record') && <th className="text-left px-4 py-3 text-gray-600 font-medium">PTR / Hostname</th>}
              {col('last_seen') && <th className="text-left px-4 py-3 text-gray-600 font-medium">Last Seen</th>}
          ...
          <tbody>
            ...
            {ips.map(ip => (
              <tr key={ip.ID} className="border-b last:border-0 hover:bg-gray-50">
                {col('address') && <td className="px-4 py-3 font-mono font-medium text-gray-800">{ip.Address}</td>}
                {col('hostname') && <td className="px-4 py-3 text-gray-500">{ip.Hostname || '—'}</td>}
                {col('status') && (
                ...
                {col('assigned_to') && <td className="px-4 py-3 text-gray-500">{ip.AssignedTo || '—'}</td>}
                {col('mac_address') && <td className="px-4 py-3 font-mono text-gray-500 text-xs">{ip.MACAddress || '—'}</td>}
                {col('ptr_record') && <td className="px-4 py-3 text-gray-500">{ip.PTRRecord || '—'}</td>}
                {col('last_seen') && (
