...
export default function RequestComments({ type, id }) {
  ...
  const loadComments = useCallback(async () => {
    try {
      ...
      setError(t('requestComments.loadError'))
    } finally {
      setLoading(false)
    }
  }, [type, id, t])
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      setSubmitError(t('requestComments.postError'))
    } finally {
      setSubmitting(false)
    }
  }
  ...
  return (
    <div className="mt-4 border-t dark:border-gray-600 pt-4">
      <h4 className="text-sm font-semibold text-gray-600 dark:text-gray-300 mb-3">{t('requestComments.title')}</h4>

      {loading ? (
        <p className="text-sm text-gray-400">{t('requestComments.loading')}</p>
      ) : error ? (
        <p className="text-sm text-red-500">{error}</p>
      ) : comments.length === 0 ? (
        <p className="text-sm text-gray-400 mb-3">{t('requestComments.noCommentsYet')}</p>
      ) : (
            ...
            <div key={c.id} className="flex items-start gap-3">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-baseline gap-2">
                  ...
                  <span className="text-xs text-gray-400">{formatRelativeTime(t, c.createdAt)}</span>
                </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-2">
        ...
        <textarea
          ...
          rows={2}
          placeholder={t('requestComments.addCommentPlaceholder')}
          value={body}
        ...
        <div className="flex justify-end">
          ...
          >
            {submitting ? t('requestComments.posting') : t('requestComments.postComment')}
          </button>
        </div>
      </form>
    </div>
