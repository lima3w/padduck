  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-4">
      <div className="flex items-center justify-between mb-3">
        ...
        >
          {building ? t('fingerprintPanel.building') : t('fingerprintPanel.rebuild')}
        </button>
      </div>

      {!fp ? (
        <p className="text-sm text-gray-400 dark:text-gray-500">{t('fingerprintPanel.noFingerprintYet')}</p>
      ) : (
        <div className="space-y-2 text-sm">
          <div>
            <div className="flex justify-between text-xs text-gray-500 mb-1">
              <span>{t('fingerprintPanel.confidence')}</span>
              <span>{pct}%</span>
            </div>
          ...
          {fp.osGuess && <div className="text-gray-600 dark:text-gray-400">{t('fingerprintPanel.osLabel')} <span className="font-medium text-gray-800 dark:text-gray-200">{fp.osGuess}</span></div>}
          {fp.vendorGuess && <div className="text-gray-600 dark:text-gray-400">{t('fingerprintPanel.vendorLabel')} <span className="font-medium text-gray-800 dark:text-gray-200">{fp.vendorGuess}</span></div>}
          {fp.openPorts?.length > 0 && (
            <div className="text-gray-600 dark:text-gray-400">
              {t('fingerprintPanel.openPortsLabel')} <span className="font-mono text-xs text-gray-800 dark:text-gray-200">{fp.openPorts.join(', ')}</span>
            </div>
          )}
