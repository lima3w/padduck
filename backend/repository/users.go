...
func (r *Repository) CreateUser(ctx context.Context, username, email string) (*models.User, error) {
	query := `INSERT INTO users (username, email, role) VALUES ($1, $2, 'user') RETURNING id, username, email, COALESCE(password_hash, '') AS password_hash, role, state, last_login_at, suspended_at, suspended_by, suspension_reason, privacy_accepted_at, privacy_accepted_version, deletion_requested_at, anonymized_at, avatar_source, organization_id, is_platform_admin, created_at, updated_at`
	row := r.db.QueryRow(ctx, query, username, email)

	user := &models.User{}
	err := row.Scan(&user.ID, &user.Username, &user.Email, &user.PasswordHash, &user.Role, &user.State, &user.LastLoginAt, &user.SuspendedAt, &user.SuspendedBy, &user.SuspensionReason, &user.PrivacyAcceptedAt, &user.PrivacyAcceptedVersion, &user.DeletionRequestedAt, &user.AnonymizedAt, &user.AvatarSource, &user.OrganizationID, &user.IsPlatformAdmin, &user.CreatedAt, &user.UpdatedAt)
	if err != nil {
		return nil, err
	}
	return user, nil
...
func (r *Repository) GetUserByID(ctx context.Context, id int64) (*models.User, error) {
	query := `SELECT id, username, email, COALESCE(password_hash, '') AS password_hash, role, state, last_login_at, suspended_at, suspended_by, suspension_reason, privacy_accepted_at, privacy_accepted_version, deletion_requested_at, anonymized_at, avatar_source, organization_id, is_platform_admin, created_at, updated_at FROM users WHERE id = $1`
...
func (r *Repository) GetUserByUsername(ctx context.Context, username string) (*models.User, error) {
	query := `SELECT id, username, email, COALESCE(password_hash, '') AS password_hash, role, state, last_login_at, suspended_at, suspended_by, suspension_reason, privacy_accepted_at, privacy_accepted_version, deletion_requested_at, anonymized_at, avatar_source, organization_id, is_platform_admin, created_at, updated_at FROM users WHERE username = $1`
...
func (r *Repository) ListAllUsers(ctx context.Context) ([]*models.User, error) {
	query := `SELECT id, username, email, COALESCE(password_hash, '') AS password_hash, role, state, last_login_at, suspended_at, suspended_by, suspension_reason, privacy_accepted_at, privacy_accepted_version, deletion_requested_at, anonymized_at, avatar_source, organization_id, is_platform_admin, created_at, updated_at FROM users ORDER BY created_at DESC`
	...
	for rows.Next() {
		user := &models.User{}
		err := rows.Scan(&user.ID, &user.Username, &user.Email, &user.PasswordHash, &user.Role, &user.State, &user.LastLoginAt, &user.SuspendedAt, &user.SuspendedBy, &user.SuspensionReason, &user.PrivacyAcceptedAt, &user.PrivacyAcceptedVersion, &user.DeletionRequestedAt, &user.AnonymizedAt, &user.AvatarSource, &user.OrganizationID, &user.IsPlatformAdmin, &user.CreatedAt, &user.UpdatedAt)
		if err != nil {
			return nil, err
		}
...
func (r *Repository) ListUsersPaginated(ctx context.Context, limit, offset int) ([]*models.User, int64, error) {
	...
	query := `SELECT id, username, email, COALESCE(password_hash, '') AS password_hash, role, state, last_login_at, suspended_at, suspended_by, suspension_reason, privacy_accepted_at, privacy_accepted_version, deletion_requested_at, anonymized_at, avatar_source, organization_id, is_platform_admin, created_at, updated_at FROM users ORDER BY created_at DESC LIMIT $1 OFFSET $2`
	...
	for rows.Next() {
		user := &models.User{}
		if err := rows.Scan(&user.ID, &user.Username, &user.Email, &user.PasswordHash, &user.Role, &user.State, &user.LastLoginAt, &user.SuspendedAt, &user.SuspendedBy, &user.SuspensionReason, &user.PrivacyAcceptedAt, &user.PrivacyAcceptedVersion, &user.DeletionRequestedAt, &user.AnonymizedAt, &user.AvatarSource, &user.OrganizationID, &user.IsPlatformAdmin, &user.CreatedAt, &user.UpdatedAt); err != nil {
			return nil, 0, err
		}
		users = append(users, user)
	}
...
func (r *Repository) SetPlatformAdmin(ctx context.Context, userID int64, val bool) error {
	_, err := r.db.Exec(ctx, `UPDATE users SET is_platform_admin = $2, updated_at = CURRENT_TIMESTAMP WHERE id = $1`, userID, val)
	return err
}

func (r *Repository) CountUsersInOrg(ctx context.Context, orgID int64) (int64, error) {
	var count int64
	err := r.db.QueryRow(ctx, `SELECT COUNT(*) FROM users WHERE organization_id = $1`, orgID).Scan(&count)
	return count, err
}
...
func (r *Repository) GetUserByEmail(ctx context.Context, email string) (*models.User, error) {
	query := `SELECT id, username, email, COALESCE(password_hash, '') AS password_hash, role, state, last_login_at, suspended_at, suspended_by, suspension_reason, privacy_accepted_at, privacy_accepted_version, deletion_requested_at, anonymized_at, avatar_source, organization_id, is_platform_admin, created_at, updated_at FROM users WHERE email = $1`
...
// GetUserAvatarData returns the stored avatar data for a user.
// Returns nil if the user has no custom avatar data.
func (r *Repository) GetUserAvatarData(ctx context.Context, userID int64) (*string, error) {
	var data *string
	err := r.db.QueryRow(ctx, `SELECT avatar_data FROM users WHERE id = $1`, userID).Scan(&data)
	return data, err
}

// UpdateUserAvatar sets the avatar_source and avatar_data for a user.
// Pass data=nil when source is "gravatar".
func (r *Repository) UpdateUserAvatar(ctx context.Context, userID int64, source string, data *string) error {
	_, err := r.db.Exec(ctx,
		`UPDATE users SET avatar_source = $2, avatar_data = $3, updated_at = CURRENT_TIMESTAMP WHERE id = $1`,
		userID, source, data,
	)
	return err
}
