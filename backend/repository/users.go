...
func (r *Repository) GetUserByUsername(ctx context.Context, username string) (*models.User, error) {
	query := `SELECT id, username, email, COALESCE(password_hash, '') AS password_hash, role, state, last_login_at, suspended_at, suspended_by, suspension_reason, privacy_accepted_at, privacy_accepted_version, deletion_requested_at, anonymized_at, avatar_source, locale, organization_id, is_platform_admin, created_at, updated_at FROM users WHERE username = $1`
...
func (r *Repository) ListAllUsers(ctx context.Context) ([]*models.User, error) {
	query := `SELECT id, username, email, COALESCE(password_hash, '') AS password_hash, role, state, last_login_at, suspended_at, suspended_by, suspension_reason, privacy_accepted_at, privacy_accepted_version, deletion_requested_at, anonymized_at, avatar_source, locale, organization_id, is_platform_admin, created_at, updated_at FROM users ORDER BY created_at DESC`
...
func (r *Repository) ListUsersPaginated(ctx context.Context, limit, offset int) ([]*models.User, int64, error) {
	...
	query := `SELECT id, username, email, COALESCE(password_hash, '') AS password_hash, role, state, last_login_at, suspended_at, suspended_by, suspension_reason, privacy_accepted_at, privacy_accepted_version, deletion_requested_at, anonymized_at, avatar_source, locale, organization_id, is_platform_admin, created_at, updated_at FROM users ORDER BY created_at DESC LIMIT $1 OFFSET $2`
...
func (r *Repository) GetUserByEmail(ctx context.Context, email string) (*models.User, error) {
	query := `SELECT id, username, email, COALESCE(password_hash, '') AS password_hash, role, state, last_login_at, suspended_at, suspended_by, suspension_reason, privacy_accepted_at, privacy_accepted_version, deletion_requested_at, anonymized_at, avatar_source, locale, organization_id, is_platform_admin, created_at, updated_at FROM users WHERE email = $1`
...
// UpdateUserLocale sets the user's preferred UI locale.
func (r *Repository) UpdateUserLocale(ctx context.Context, userID int64, locale string) error {
	_, err := r.db.Exec(ctx,
		`UPDATE users SET locale = $2, updated_at = CURRENT_TIMESTAMP WHERE id = $1`,
		userID, locale,
	)
	return err
}
