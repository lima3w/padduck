...
func TestAuthenticateUser_Integration(t *testing.T) {
	...
	result, err := svc.Ops.Identity.AuthenticateUser(ctx, "pw-user", "original-password", "10.0.0.1", "ua")
	...
	_, err = svc.Ops.Identity.AuthenticateUser(ctx, "pw-user", "wrong", "10.0.0.1", "ua")
	assert.ErrorContains(t, err, "invalid password")
	_, err = svc.Ops.Identity.AuthenticateUser(ctx, "ghost", "whatever", "10.0.0.1", "ua")
	assert.ErrorContains(t, err, "user not found")

	// Empty credentials are rejected up front.
	_, err = svc.Ops.Identity.AuthenticateUser(ctx, "", "", "10.0.0.1", "ua")
...
func TestAuthenticateUser_AccountStates_Integration(t *testing.T) {
	...
	for state, wantErr := range map[string]error{
		...
		_, err := svc.Ops.Identity.AuthenticateUser(ctx, "pw-user", "original-password", "10.0.0.1", "ua")
...
func TestAuthenticateUser_Lockout_Integration(t *testing.T) {
	...
	for i := 0; i < 5; i++ {
		_, err := svc.Ops.Identity.AuthenticateUser(ctx, "pw-user", "wrong", "10.0.0.9", "ua")
		assert.ErrorContains(t, err, "invalid password")
	}

	locked, lockout, err := svc.Ops.Identity.IsAccountLocked(ctx, userID)
	require.NoError(t, err)
	...
	_, err = svc.Ops.Identity.AuthenticateUser(ctx, "pw-user", "original-password", "10.0.0.9", "ua")
...
func TestSessionManagement_Integration(t *testing.T) {
	...
	tokenA, err := svc.Ops.Identity.CreateWebSession(ctx, userID, "10.0.0.1", "Mozilla/5.0 (X11; Linux x86_64) Firefox/120.0")
	require.NoError(t, err)
	_, err = svc.Ops.Identity.CreateWebSession(ctx, userID, "10.0.0.2", "curl/8.0")
	require.NoError(t, err)

	sessions, err := svc.Ops.Identity.ListUserSessions(ctx, userID)
	require.NoError(t, err)
	require.Len(t, sessions, 2)

	// RevokeSessionByID enforces ownership.
	assert.Error(t, svc.Ops.Identity.RevokeSessionByID(ctx, userID+999, sessions[0].ID))
	require.NoError(t, svc.Ops.Identity.RevokeSessionByID(ctx, userID, sessions[0].ID))
	sessions, err = svc.Ops.Identity.ListUserSessions(ctx, userID)
	require.NoError(t, err)
	assert.Len(t, sessions, 1)

	// RevokeSession by raw token enforces ownership too.
	assert.Error(t, svc.Ops.Identity.RevokeSession(ctx, userID+999, tokenA))
	assert.Error(t, svc.Ops.Identity.RevokeSession(ctx, userID, "unknown-token"))
	...
	_, _, err = svc.Ops.Identity.ValidateSession(ctx, tokenA)
	assert.Error(t, err)

	require.NoError(t, svc.Ops.Identity.UpdateLastLogin(ctx, userID))

	throttled, err := svc.Ops.Identity.IsIPThrottled(ctx, "10.0.0.1")
	require.NoError(t, err)
...
func TestValidateAPIToken_Integration(t *testing.T) {
	...
	raw, err := svc.Ops.Identity.GenerateAPIToken(ctx, userID, "validate-me", "read", 30)
	require.NoError(t, err)

	user, tok, err := svc.Ops.Identity.ValidateAPIToken(ctx, raw, "10.0.0.5")
	require.NoError(t, err)
	assert.Equal(t, userID, user.ID)
	assert.Equal(t, "read", tok.Scope)

	_, _, err = svc.Ops.Identity.ValidateAPIToken(ctx, "unknown", "10.0.0.5")
	...
	require.NoError(t, err)
	_, _, err = svc.Ops.Identity.ValidateAPIToken(ctx, expiredRaw, "10.0.0.5")
	...
	_, _, err = svc.Ops.Identity.ValidateAPIToken(ctx, raw, "10.0.0.5")
	assert.ErrorContains(t, err, "grace period has expired")

	require.NoError(t, svc.Ops.Identity.CleanupExpiredTokens(ctx))
}
