...
func TestCreateSection_EmptyName(t *testing.T) {
	...
	_, err := svc.Ops.IPAM.CreateNetwork(ctx, "", "some description", 1)
	assert.Error(t, err)
...
func TestCreateSection_ValidName_PassesValidation(t *testing.T) {
	...
	assert.Panics(t, func() {
		_, _ = svc.Ops.IPAM.CreateNetwork(ctx, "My Network", "description", 1)
	})
}
...
func TestGetSection_InvalidID(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			_, err := svc.Ops.IPAM.GetNetwork(ctx, tt.id)
			assert.Error(t, err)
...
func TestUpdateSection_InvalidID(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			_, err := svc.Ops.IPAM.UpdateNetwork(ctx, tt.id, "Some Name", "desc")
			assert.Error(t, err)
			assert.Contains(t, err.Error(), "invalid section ID")
		})
	}
...
func TestUpdateSection_EmptyName(t *testing.T) {
	...
	_, err := svc.Ops.IPAM.UpdateNetwork(ctx, 1, "", "desc")
	assert.Error(t, err)
	assert.Contains(t, err.Error(), "section name is required")
}
...
func TestDeleteSection_InvalidID(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			err := svc.Ops.IPAM.DeleteNetwork(ctx, tt.id)
			assert.Error(t, err)
			assert.Contains(t, err.Error(), "invalid section ID")
		})
	}
