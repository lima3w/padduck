...
func (s *ImportService) ExportV2MigrationBundle(ctx context.Context) ([]byte, string, string, error) {
	...
	readme := []byte(strings.TrimSpace(`# IPAM Next v2 Migration Bundle
...
- data/ipam-v1-export.json: structured v1 export for migration tooling.
- data/ipam-v1-export.csv: human-readable fallback export.
...
`) + "\n")
	...
	files := []migrationBundleFile{
		bundleFile("data/ipam-v1-export.json", "application/json", jsonData),
		bundleFile("data/ipam-v1-export.csv", "text/csv", csvData),
	...
	if err := addZipFile(zw, "data/ipam-v1-export.json", jsonData); err != nil {
		return nil, "", "", err
	}
	if err := addZipFile(zw, "data/ipam-v1-export.csv", csvData); err != nil {
		return nil, "", "", err
	}
	...
	filename := fmt.Sprintf("ipam-v2-migration-bundle-%s.zip", now.Format("20060102-150405"))
